...
Describe '507 Labs' {
  ...
  Context 'Lab 1.3 - AWS' -Skip:$skipAWS {
    ...
    It 'Part 1 - AWS CLI returns instances' {
    ...
    It 'Part 1 - PowerShell module returns instances' {
      (Get-EC2Instance).Count | Should -BeGreaterOrEqual 5
    }

    It 'Part 1 - Get-AWSCmdletName returns multiple results' {
      (Get-AWSCmdletName -ApiOperation describeinstances).Count | 
        Should -BeGreaterOrEqual 3
    }

    It 'Part 1 - Get-AWSCmdletName with service returns correct results' {
    ...
    It 'Part 1 - Get-AWSCmdletName with CLI command returns correct results' {
  ...
  Context 'Lab 2.1' {
    ...
    It 'Part 2 - Student and rciadmin are the only enabled users' {
      ...
      $enabledUsers.Name | Should -Contain 'rciadmin'      
    }

    It 'Part 2 - WDAGUtilityAccount and student are the only PasswordExpires users' {
      $passwordExiresUsers = (Get-LocalUser | Where-Object passwordExpires -NE $null)
      $passwordExiresUsers.Count | Should -Be 2
      $passwordExiresUsers.Name | Should -Contain 'WDAGUtilityAccount' 
      $passwordExiresUsers.Name | Should -Contain 'student' 
    }

    It 'Part 3 - Not enabled users' {
      $enabledUsers = (Get-LocalUser | Where-Object enabled -EQ $false)
      $enabledUsers.Count | Should -Be 3
    }
  }

  Context 'Lab 2.2' {
    It 'Part 1 - Build number is 26100' {
      (Get-CimInstance Win32_OperatingSystem).BuildNumber | Should -Be 26100
    }
  ...
  Context 'Lab 2.3: AWS VPN to DC' -Skip:$skipDC { 
    ...
    It 'Part 5 - Greater Than 1006 AD users returned' {
      (Get-ADUser -Filter * -Server 507dc -Credential $cred).Count | Should -BeGreaterOrEqual 1006
    }
